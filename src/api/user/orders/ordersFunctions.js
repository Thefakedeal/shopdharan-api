...
async function getAddress(address_id) {
  const addressquery = await db.query(
    `SELECT address_id, city_id, street_name, details 
    FROM address
...
function getDeliveryCharge({ sameCity = 0, differentCity = 0 }) {
  const insideCityPrice = parseInt(process.env.PRICE_INSIDECITY);
  const outsideCityPrice = parseInt(process.env.PRICE_OUTSIDECITY);
  const deliveryCharge =
    insideCityPrice * sameCity + outsideCityPrice * differentCity;
